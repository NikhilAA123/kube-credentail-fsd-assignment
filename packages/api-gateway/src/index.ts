...
app.use('/issuance', verifyTokenMiddleware, attachUserHeader, createProxyMiddleware({
  target: process.env.ISSUANCE_URL || 'http://localhost:4001',
  changeOrigin: true,
  pathRewrite: { '^/issuance': '' }
}));

app.use('/verification', verifyTokenMiddleware, attachUserHeader, createProxyMiddleware({
  target: process.env.VERIFICATION_URL || 'http://localhost:4002',
  changeOrigin: true,
